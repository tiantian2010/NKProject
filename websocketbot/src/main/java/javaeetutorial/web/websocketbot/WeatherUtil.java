...
public class WeatherUtil {
    ...
    public static List<Map<String, Object>> getWeekWeatherMap(String Cityid)
        ...
        // 连接中央气象台的API
        System.out.println("进入函数，Cityid：" + Cityid);
//                 URL url = new URL("http://m.weather.com.cn/atad/" + Cityid + ".html");
        ...
        try {
            ...
            JSONObject info = jsonData.getJSONObject("forecast");
            JSONArray info2 = jsonData.getJSONArray("index");
            System.out.println("---:"+((JSONObject)(info2.get(1))).getString("details"));
            for (int i = 1; i <= 6; i++) {
                // 得到未来6天的日期
                ...
                map.put("weather", info.getString("weather" + i));// 天气
                map.put("temp", info.getString("temp" + i));// 温度
                map.put("wind", info.getString("wind" + i));// 风况
                map.put("fx", info.getString("fx1"));// 风向
                map.put("fl", info.getString("fl" + i));// 风速
                map.put("index_d", ((JSONObject)(info2.get(1))).getString("details"));// 今天的穿衣指数
                map.put("index_dx", ((JSONObject)(info2.get(0))).getString("details"));
                map.put("index_dl", ((JSONObject)(info2.get(2))).getString("details"));
                map.put("index_dll", ((JSONObject)(info2.get(3))).getString("details"));
        ...
        try {
            ...
            for (int j = 0; j < listData.size(); j++) {
                ...
                System.out.println(wMap.get("city") + "\t" + wMap.get("date_y")
                    ...
                    + "\t" + wMap.get("index_uv") + "\t" + wMap.get("index_d")+ wMap.get("index_dl")+ wMap.get("index_dll"));
            }
