    ...
    public void message(final Session session, Message msg) {
        ...
        if (msg instanceof JoinMessage) {
            ...
            sendAll(session, new InfoMessage(jmsg.getName() + 
                    " has joined the chat"));
            sendAll(session, new ChatMessage("TT", jmsg.getName(),
                    "Hi there!!"));
            ...
            if (cmsg.getTarget().compareTo("TT") == 0) {
                ...
                mes.submit(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            sendAll(session, new ChatMessage("TT",
                                    cmsg.getName(), resp));
                        } catch (IOException ex) {
                            Logger.getLogger(BotEndpoint.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
                });
            }
